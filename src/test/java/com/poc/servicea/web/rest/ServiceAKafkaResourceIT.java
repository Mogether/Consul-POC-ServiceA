...
import com.poc.servicea.config.EmbeddedKafka;
import com.poc.servicea.config.KafkaSseConsumer;
import com.poc.servicea.config.KafkaSseProducer;
...
class ServiceAKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/service-a-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/service-a-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/service-a-kafka/unregister"));
                return;
            }
        }
